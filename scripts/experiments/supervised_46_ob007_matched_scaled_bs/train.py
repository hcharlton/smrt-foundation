"""
Data scaling with OB007 pretrained encoder (exp 29), exp 42's single-stage
schedule (AdamW lr=3e-3 all params, cosine with warmup), and exp 44's
per-size batch sizing.

Batch size per run: min(bs_cap, max(bs_floor, n_train // bs_k)).
At n >= bs_cap * bs_k the batch hits the cap and the run is identical to
exp 42. Divergence from exp 42 is entirely at small/mid n.
"""
...
def train_one_size(rank, train_size, config, c, experiment_dir, tb_dir):
    ...
    csv_writer.writerow([
        'train_size', 'train_bs', 'eval_point', 'step',
        'train_loss', 'val_loss',
