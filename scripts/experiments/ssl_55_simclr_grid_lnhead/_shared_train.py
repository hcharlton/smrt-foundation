...
Derived from ssl_54_simclr_grid_yoran/_shared_train.py with two changes:
  ...
  1. **LayerNorm projection head**: instantiates `SimCLRSmrtLN` rather
     than `SimCLRSmrt`. The new class swaps `MLPProjectionHead` for
     `MLPProjectionHeadLN` (LN between Linear and GELU on hidden layers,
  ...
  2. **Non-finite-grad skip**: when `accelerator.clip_grad_norm_` returns
     a non-finite value, `optimizer.step()` and `scheduler.step()` are
...
All other SSL hyperparameters, augmentations, schedule, dataset, probe,
and checkpointing logic stay identical to ssl_54.
