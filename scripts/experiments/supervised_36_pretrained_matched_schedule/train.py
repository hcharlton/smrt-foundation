"""
Data scaling with pretrained encoder (exp 25) using exp 33's exact schedule.

Single-stage training: AdamW lr=3e-3 for all params, single cosine schedule
over 400k steps. The ONLY difference from exp 33 is that the encoder is
initialized from the exp 25 autoencoder checkpoint instead of randomly.
This isolates the effect of pretraining without confounding from different
optimizer schedules (cf. exp 34 which uses two-stage differential LR).
"""
...
def train_one_size(rank, train_size, config, c, steps_per_size, eval_steps,
    ...
    # Optimizer + schedule — identical to exp 33.
    ...
    # Per-size CSV — same format as exp 33.
