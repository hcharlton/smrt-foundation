"""
Data scaling with OB007 pretrained encoder (exp 29) using exp 33's exact schedule.

Exp 29: SmrtAutoencoder pretrained on 25G of OB007 data at ctx=128, 63% probe
top-1. Single-stage training: AdamW lr=3e-3 for all params, single cosine
schedule over 400k steps. The ONLY difference from exp 33 is that the encoder
is initialized from the exp 29 checkpoint instead of randomly. PE buffer is
reinitialized due to ctx 128→32 mismatch (handled by load_pretrained_encoder).
"""
