...
## 2026-05-19: Four-Way Partition Implementation — Equal Vals at `cpg_*_v3.memmap/`
...
2. **Fresh output path `cpg_{pos,neg}_v3.memmap/`**, not in-place regeneration of v2. Costs ~210 GB of extra disk but keeps v2 intact as a fallback and makes every config switch from v2→v3 visible in diff. The "zero-cost rename" property the original sketch promised is forfeit once the ratio changes, so an in-place overwrite would be both destructive and not actually rename-equivalent — the fresh path is the honest framing.
...
**Follow-ups deferred:**
- `workflow.py:395-455` — `memmap_cpg_conversion` still passes `--val_pct` and the CONFIG dict has no `cpg_pos_v3` / `cpg_neg_v3` registry entries. Will add a v3-aware task variant when the regeneration is queued on Gefion.
- Experiment configs (ssl_58 / ssl_59 / ssl_60 / supervised_53): `probe_pos_val` / `probe_neg_val` and `pos_data_val` / `neg_data_val` keys need to point at `…/val1` and `…/val2` respectively, and new `pos_data_test` / `neg_data_test` keys at `…/val3` on the supervised side. Will be done at launch time, one experiment at a time, so each switch is a single PR-equivalent diff.
...
- `tests/test_zarr_to_methyl_memmap_v2.py` — currently hardcodes `train`/`val` and asserts the 80/20 fraction. Will be reworked to validate the four-way layout once a first regeneration confirms the output is well-formed.
