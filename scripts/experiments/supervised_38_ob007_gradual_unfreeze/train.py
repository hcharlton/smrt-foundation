...
Same two-stage setup as exp 35 but with a key change to stage 2: instead of
jumping the encoder LR to 3e-4 immediately, it warms up from 1e-5 to 3e-4
over 100k steps. This protects pretrained features from being disrupted by
large early gradients when the encoder is first unfrozen.

Stage 1 (steps 1-100k): Frozen encoder, head-only at lr=3e-3
Stage 2 (steps 100k-400k): Unfrozen, encoder LR 1e-5 → 3e-4 over 100k steps
  then cosine decay. Head LR same as exp 35 (cosine warmup to 3e-3).
"""
...
def make_cosine_warmup_lambda(total_steps, warmup_steps, min_lr_ratio=0.0):
    """Create a LR lambda: linear warmup from min_lr_ratio to 1.0, then cosine decay to 0."""
    def lr_lambda(step):
...
def train_one_size(rank, train_size, config, c, steps_per_size, eval_steps,
    ...
    """Train a single training size on GPU `rank` with gradual encoder unfreezing."""
    ...
    for step in range(1, steps_per_size + 1):
        # --- Stage transition: gradual encoder unfreezing ---
        if step == frozen_steps + 1:
            stage = 2
            for p in model.encoder.parameters():
                p.requires_grad = True

            stage2_steps = steps_per_size - frozen_steps
            encoder_warmup = ft['encoder_warmup_steps']
            ...
            # Per-group schedules: encoder gets slow warmup, head gets standard warmup
...
def main():
    ...
    print(f"Encoder warmup: {config['finetune']['encoder_start_lr']} → "
          f"{config['finetune']['encoder_lr']} over "
          f"{config['finetune']['encoder_warmup_steps']:,} steps")
