"""
Facet plot of validation accuracy trajectories (val_accuracy vs step)
for experiments 37 and 38. One facet per training dataset size; two
lines per facet (one per experiment). Independent x and y scales per
facet so each trajectory uses the full plotting area.
"""
...
def main(output_path):
    ...
    chart = alt.Chart(df).mark_line().encode(
        ...
        alt.Y('val_accuracy:Q', scale=alt.Scale(zero=False)).title(config.get('y_label', 'Top-1 Accuracy')),
