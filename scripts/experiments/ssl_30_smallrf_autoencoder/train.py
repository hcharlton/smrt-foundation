"""
Masked autoencoder pretraining on CpG data with the Small-RF CNN variant.

Delta vs exp 25: swap SmrtAutoencoder -> SmrtAutoencoderSmallRF (4-block CNN,
receptive field 27 instead of 107). Everything else — data, normalization,
optimizer, loss, probe — is identical, so the delta in probe/fine-tune
accuracy is attributable to the receptive field alone.

Hypothesis: restoring RF < context restores per-latent locality, which
masked-prediction SSL objectives implicitly depend on. If this closes
the 3pp gap between exp 27 (79% fine-tune) and exp 20 (82% supervised),
the RF mismatch was the dominant remaining bottleneck.
"""
...
def main():
    ...
    DEFAULT = {
        ...
        'batch_size': 1024, 'epochs': 12, 'ds_limit': 2_000_000,
        'max_lr': 3e-4, 'p_mask': 0.15, 'mask_size': 5,
    ...
    # Load CpG data (pos + neg combined, labels discarded)
    pos_dir = config.get('ssl_pos_dir')
    neg_dir = config.get('ssl_neg_dir')
    limit_per_dir = c['ds_limit'] // 2

    ds_pos = ShardedMemmapDataset(pos_dir, limit=limit_per_dir)
    ds_neg = ShardedMemmapDataset(neg_dir, limit=limit_per_dir)
    ds = ConcatDataset([ds_pos, ds_neg])

    if accelerator.is_main_process:
        print(f"CpG dataset: {len(ds)} samples ({len(ds_pos)} pos + {len(ds_neg)} neg, labels discarded)")

    # Compute normalization from CpG data (same distribution as probe)
    norm_ds = LabeledMemmapDataset(pos_dir, neg_dir, limit=c['ds_limit'])
    ssl_norm = KineticsNorm(norm_ds, max_samples=16_384, log_transform=True)
    del norm_ds

    if accelerator.is_main_process:
        print(f"CpG norm — means: {ssl_norm.means}, stds: {ssl_norm.stds}")
    ...
    # Fail-fast: the whole point of this experiment is that RF < context.
    # If someone copies this train.py and bumps the context without also
    # swapping back to the full-size encoder, the locality premise breaks
    # silently. Make it loud.
    assert model.encoder.cnn.r0 < c['context'], (
        f"SmrtAutoencoderSmallRF receptive field ({model.encoder.cnn.r0}) must be "
        f"smaller than context ({c['context']}) for the locality hypothesis to hold. "
        f"If you need a larger context, use the default SmrtAutoencoder instead."
    )
