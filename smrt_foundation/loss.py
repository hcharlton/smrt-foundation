...
def cpc_loss(c_target, predictions):
    ...
    for k in range(1, K + 1):
        ...
        # We can only predict up to L-k steps
        L_valid = L - k
        if L_valid <= 0:
            continue
            
        # Get contexts c_t for t = 0 to L-k-1
        # These are the *inputs* to the predictors W_k
        ...
        # We contrast c_t's prediction against ALL z_j in the batch
        # This includes z from other sequences and other time steps
        # scores = [B*L_valid, B*L_valid]
        # calculate similarity scores with the dotproduct
        scores = torch.matmul(c_t_flat, z_target_k_flat.T)
        
        # Labels: the positive sample is the one at the same (b, t) index
        # which corresponds to the diagonal of the `scores` matrix
