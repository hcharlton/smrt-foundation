"""
Data scaling with OB007 pretrained encoder (exp 29), gradual unfreezing,
epoch-capped step budgets, and per-size batch sizing to preserve SGD
noise at small n_train.

Batch size per run: min(bs_cap, max(bs_floor, n_train // bs_k)).
With defaults bs_cap=4096, bs_floor=64, bs_k=8:
  n=100..500   -> bs=64   (real subsampling instead of full-set echoes)
  n=1000       -> bs=125
  n=8000       -> bs=1000
  n>=32000     -> bs=4096 (cap, matches exp 43)

Two-stage unfreezing identical to exp 43:
...
DEFAULT = {
    ...
    'batch_size': 4096, 'bs_floor': 64, 'bs_k': 8,
...
def compute_batch_size(n_train, bs_cap, bs_floor, bs_k):
    """Scale batch size with dataset size to keep SGD stochastic at small n.

    Returns min(bs_cap, max(bs_floor, n_train // bs_k)) clamped to n_train.
    """
...
def train_one_size(rank, train_size, config, c, experiment_dir, tb_dir):
    ...
    print(f"{tag} Train: {n_train}, Val: {len(val_x)}, "
          f"Train bs: {train_bs} (cap={c['batch_size']}, floor={c['bs_floor']}, k={c['bs_k']}), "
    ...
    print(f"{tag} Train bs: {train_bs}, Eval bs: {eval_bs}")
...
def main():
    ...
    print(f"Batch size: cap={c['batch_size']}, floor={c['bs_floor']}, k={c['bs_k']}")
    ft = config['finetune']
